...
public final class ControllerTxt {
    ...
    public static void searchTxtFlight(Company c) {
        ...
        ArrayList<Route> r = c.searchRouteForCityAndAirport(s1, s2);       
        if (r.size()!=0) {
            if (r.size() > 1) {
                ...
                ArrayList<Flight> arrayFlight = c.searchFlights(r.get(0), date);
                if (!arrayFlight.isEmpty()) {
                    ...
                    ArrayList<Flight> calendar = c.calendarFlight(r.get(0));
                    if (calendar.isEmpty()) {
